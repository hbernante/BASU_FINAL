...
export default function ReservationView() {
  ...
  const onSubmit = (ev) => {
    ...
    let res = null;
    if (id) {
      res = axiosClient.put(`/reservation/${id}`, payload);
    } else {
      res = axiosClient.post("/reservation", payload);
    }
    ...
    res
      .then((res) => {
        ...
        if (id) {
          showToast("The reservation was updated");
        } else {
          showToast("The reservation was created");
        }
      })
      .catch((err) => {
        if (err && err.response) {
  ...
  useEffect(() => {
    if (id) {
      setLoading(true);
      axiosClient.get(`/reservation/${id}`).then(({ data }) => {
  ...
  return (
    <PageComponent
      title={!id ? "Create new Reservation" : "Update Reservation"}
      buttons={
        <div className="flex gap-2">
          <TButton
            color="green"
            href={`/reservation/public/${reservation.slug}`}
          >
